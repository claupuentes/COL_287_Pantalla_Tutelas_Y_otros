...
/* Verificación mensaje Producto PRE */
...
public class Mensaje_prod_PRE {
...

	{
		...
		ngWebDriver.waitForAngularRequestsToFinish();
		//Thread.sleep(2000);
		String documentNumber = readFile.getCellValue(filepath,"Mens_prod_PRE", 1, 2);
	...
	
	{
		...
		ExtentTest test4 = extent.createTest("VERIFICACION MENSAJE NO TIENE PRE","PRUEBAS AUTOMATIZADAS COLMEDICA");
		Assert.assertTrue(driver.findElement(By.xpath("//*[@id='swal2-title']")).getText().contains("Usuario No tiene prepago vigente"));
        test4.pass("MENSAJE MODAL EMERGENTE  Usuario No tiene prepago vigente");//(4)
        ngWebDriver.waitForAngularRequestsToFinish();
        
		Assert.assertTrue(driver.findElement(By.xpath("//*[@id='swal2-content']")).getText().contains("No se puede realizar la autorización por este módulo."));
        test4.pass("MENSAJE MODAL EMERGENTE  No se puede realizar la autorizacion por este modulo.");//(4)
        Thread.sleep(2000);
        ngWebDriver.waitForAngularRequestsToFinish();
